...
fn camera_stream(sender: glib::Sender<((Vec<u8>))>, cambool: Arc<AtomicBool>, uuid_sender: glib::Sender<String>, authbool_clone: Arc<AtomicBool>){
    thread::spawn(move||{
        ...
        let mut qr_detector = objdetect::QRCodeDetector::default().expect("detector failed");
        let mut qr_result = opencv::types::VectorOfPoint::new();
        let mut qr_mat = Mat::default();
        ...
        loop {
            if !cambool.load(Ordering::Relaxed){
                ...
                if timer.elapsed() > Duration::from_secs(1) && authbool_clone.load(Ordering::Relaxed) == false{
                    let ret = qr_detector.detect_and_decode(&mat,&mut qr_result, &mut qr_mat).unwrap();
                    if !ret.is_empty(){
                        // Checking if the qr code is valid if so send to main thread
                        uuid_sender.send(String::from_utf8(ret).unwrap());
                    }
                    timer = Instant::now();
                }
...
fn draw_ui(app: &Application) {
    ...
    // atomic bool to if user has been authenticated
    ...
    uuid_receiver.attach(None,move|data|{
        ...
        for person in auth_list.access{
            if person.uuid.to_string() == data{
                authbool.store(true,Ordering::Relaxed);
                ol_clone.set_label("QR Code gescannt Fotos werden in 3 sekunden gemacht");
                ...
                let thread = thread::spawn(move||{
                    ...
                    for i in 0..3{
                        ...
                        imwrite(&format!("/Users/benediktkarli/Desktop/physik/v2/src/{:?}-{:?}.jpg",person.uuid.to_owned(),i).to_owned(),&dest,&empty_array);
    ...
    finished_receiver.attach(None,move|uuid: i32|{
        for i in 0..3{
            let t = ol.clone();
            let u = mb.clone();
            let ui = abc.clone();
